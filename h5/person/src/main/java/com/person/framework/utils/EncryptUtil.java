...
public class EncryptUtil {
	...
	public static class UserTokenData {
		...
		public UserTokenData(String type, Integer userId, String userName,
				Date generateDate) {
			super();
			this.type = type;
			this.userId = userId;
		...
		public UserTokenData(String joinedText) {
			String[] split = StringUtil.split(joinedText);
			this.type = split[0];
			this.generateDate = DateUtil.parse(split[1],
		...
		/**
		 * Check if within validSeconds from generateDate to now.
		 * 
		 * @param validSeconds
		 * @return
		 */
		public boolean isValid(long validSeconds) {
			return (DateUtil.getCurrentDate().getTime() - this.generateDate
					.getTime()) <= validSeconds * 1000;
		}

		@Override
		public String toString() {
			return StringUtil.join(new Object[] { type,
					DateUtil.formatDateTime(generateDate), userId, userName });
		}
	}
	 ...
	 *            用户登录名
	 */
	public static String generateToken(UserTokenData tokenData) {

		return encodeToken(tokenData.toString());
	}

	public static UserTokenData recoverToken(String tokenCipher) {
		String decryptedText = decodeToken(tokenCipher);
		return new UserTokenData(decryptedText);
