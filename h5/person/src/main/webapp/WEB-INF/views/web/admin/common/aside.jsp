...
<aside class="Hui-aside">
	...
	<div class="menu_dropdown bk_2">
		<c:forEach var="menu" items="${_userResource_.sourceTree}"  varStatus="varStatus1">
			<c:if test="${not empty menu.children}">
				<dl id="menu-article">
					<dt>
						<i class="Hui-iconfont">${menu.iconClass }</i> ${menu.text}<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i>
					</dt>
					<dd>
						<ul>
							<c:forEach var="childMenu" items="${menu.children}" varStatus="varStatus">
								<li><a _href="${childMenu.url}" data-title="${childMenu.text}" href="javascript:void(0)">${childMenu.text}</a></li>
							</c:forEach>
						</ul>
					</dd>
				</dl>
