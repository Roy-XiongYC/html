...
<body>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="add('添加','${webroot}/admin/blogXc/addPage?id=','600','600')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加</a></span></div>
	<table id="table1" class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="6">列表</th>
			</tr>
			<tr class="text-c">
										<th name="userName">用户名</th>
										<th name="img">相册图片地址</th>
										<th name="imgWidth">相册图片宽度</th>
										<th name="imgHeight">相册图片高度</th>
...
"sAjaxSource" : '${webroot}/admin/blogXc/data',
"aoColumns" :[

	{"mData" : "userName",'sClass' : 'center'},				
	{"mData" : "img",'sClass' : 'center'},				
	{"mData" : "imgWidth",'sClass' : 'center'},				
	{"mData" : "imgHeight",'sClass' : 'center'},				
...
"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
	...
	$('td:eq(1)',nRow).html('<div class="zoomerImg"><img style="width: 40px;height: 40px;" src="'+aData.img+'" alt="" /></div>');
	$('td:eq(4)',nRow).html(app.inputDate.myformatterTime(new Date(aData.createTime)));
	$('td:eq(5)',nRow).html(app.addButton.update('edit',aData.id,'600','600')+app.addButton.del('del',aData.id));
	return nRow;
},
...
function edit(title,id,w,h){
	layer_show(title,'${webroot}/admin/blogXc/addPage?id='+id,w,h);
}
function doQuery(){
